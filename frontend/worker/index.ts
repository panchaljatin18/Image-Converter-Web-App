...
const documentFormats = ["pdf"]; // target formats
const archiveFormats = ["zip", "tar"];
...
const worker = new Worker("conversion-jobs", async (job: Job) => {
  ...
  const cleanFormat = targetFormat.startsWith(".") ? targetFormat.slice(1).toLowerCase() : targetFormat.toLowerCase();
  ...
  try {
    ...
    if (imageFormats.includes(cleanFormat)) {
      await processImage(inputFilePath, outputPath, cleanFormat, options || {});
    } else if (videoFormats.includes(cleanFormat) || audioFormats.includes(cleanFormat)) {
      ...
      throw new Error(`Target format '${cleanFormat}' is not supported yet.`);
    }
    ...
    if (fs.existsSync(outputPath)) {
        try {
            ...
            console.error(`[Job ${jobId}] Failed to cleanup output file:`, cleanupErr);
        }
    }
