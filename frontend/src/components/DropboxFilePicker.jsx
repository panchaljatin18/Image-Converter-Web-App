...
export default function DropboxFilePicker({ onFileSelected, onCancel }) {
  const { data: session } = useSession();
  const { user } = useAuth();
  
  const [files, setFiles] = useState([]);
  const [currentFolderId, setCurrentFolderId] = useState(""); // empty string represents root in Dropbox
  ...
  const fetchFiles = useCallback(async (folderId, query = "", pageToken = "") => {
    ...
    setError("");
    try {
      const data = await dropboxService.getFiles(token, folderId, query, pageToken);
  ...
  const handleSelectFile = async (file) => {
    if (file.mimeType === "application/vnd.google-apps.folder") {
      ...
      setCurrentFolderId(file.id); // In Dropbox, file.id contains the lowercase path (e.g. "/documents")
      return;
    }
    ...
    setError("");
    try {
      const blob = await dropboxService.downloadFile(token, file.id);
      ...
      setError(err.message || "Failed to download Dropbox file");
    } finally {
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            <h3 style={{ margin: 0, fontSize: "1.1rem", fontWeight: 600 }}>Dropbox Files</h3>
          </div>
          {onCancel && (
            <button
              onClick={onCancel}
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
        ...
        {isDownloading && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "12px", padding: "40px 0" }}>
            ...
            <p style={{ margin: 0, fontSize: "0.9rem", color: "var(--text-muted)" }}>Importing file from Dropbox...</p>
          </div>
        )}
                ...
                >
                  <div
                    style={{
                      ...
                      height: "48px",
                      borderRadius: "10px",
                      background: isFolder ? "rgba(0, 97, 254, 0.1)" : "rgba(255, 255, 255, 0.04)",
                      color: isFolder ? "#0061FE" : "var(--text-muted)",
                      position: "relative",
                      overflow: "hidden"
                    }}
                  >
