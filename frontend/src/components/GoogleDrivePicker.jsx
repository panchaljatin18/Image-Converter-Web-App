...
export default function GoogleDrivePicker({ onFileSelected, onCancel }) {
  ...
  const fetchFiles = useCallback(async (folderId, query = "", pageToken = "") => {
    ...
    setError("");
    try {
      const data = await googleDriveService.getFiles(token, folderId, query, pageToken);
  ...
  const handleSelectFile = async (file) => {
    ...
    setError("");
    try {
      const blob = await googleDriveService.downloadFile(token, file.id);
            ...
            {files.map((file) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {isFolder ? <Folder size={24} /> : file.thumbnailLink ? (
                      <img src={file.thumbnailLink} alt="" referrerPolicy="no-referrer" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    ) : <File size={24} />}
                  </div>
