...
export default function AdminLayout({ children }) {
  ...
  // Login form states
  const [username] = useState("jmpanchal394@gmail.com");
  ...
  if (!isAuthorized) {
    return (
      <div className="admin-body min-h-screen flex items-center justify-center p-4">
        <div className="admin-card max-w-md w-full p-8 space-y-6 relative overflow-hidden border-[#2a2a38] shadow-2xl bg-[#16161f]/90 backdrop-blur-md">
          ...
          <div className="flex flex-col items-center text-center space-y-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#9494a3] mt-1 font-['Outfit']">Enter credentials to access the CMS console</p>
            </div>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4 text-left">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                value={username}
                disabled
                className="admin-input opacity-75 cursor-not-allowed border-[#2a2a38] font-semibold text-white"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="admin-btn admin-btn-primary w-full justify-center mt-6"
            >
              {loading ? (
                <>
  ...
  return (
    <div className="admin-body min-h-screen flex flex-col md:flex-row" style={{ paddingTop: "72px" }}>
      <aside className="admin-sidebar w-full md:w-64 shrink-0 flex flex-col justify-between py-6 px-4 md:h-[calc(100vh-72px)] sticky top-[72px] z-20">
        <div className="space-y-8">
          {/* Logo Header */}
          <div className="px-3 flex items-center gap-2.5">
            <span className="w-9 h-9 rounded-xl bg-gradient-to-tr from-[#8b5cf6] to-[#06b6d4] flex items-center justify-center font-bold text-white text-base">
              CG
            </span>
            <div>
