...
function CallbackContent() {
  ...
  useEffect(() => {
    const exchangeCode = async () => {
      ...
      if (errorParam) {
        setStatus("error");
        setErrorMessage(errorParam || "Dropbox authentication rejected.");
        return;
      }
      ...
      try {
        ...
        const res = await dropboxService.connect(token, code, redirectUri);
      ...
      >
        {status === "loading" && (
          <>
            ...
            <h2 style={{ margin: "0 0 10px 0", fontSize: "1.5rem", fontWeight: 700 }}>Connecting Dropbox</h2>
        ...
        {status === "success" && (
          <>
            ...
            <p style={{ margin: 0, color: "var(--text-muted)", fontSize: "0.95rem" }}>
              Your Dropbox is now connected. Redirecting you back to uploader...
            </p>
          </>
        )}
